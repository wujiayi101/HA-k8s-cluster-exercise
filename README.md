...
1. Create an AWS IAM user with read permissions to both secrets, generate an access key 
1. Create a Kubernetes secret to hold the access key. Kubernetes uses access key in this secret to pull external secrets to the cluster.
    ```
    ...
    ```
1. Create [external secrets](https://external-secrets.io/)
    ```
    ./install_eso.sh
    kubectl apply -f n8n/external-secrets/ -n n8n
    ```
